...
export async function getUserBalance( userId:number ): Promise<number> {
    ...
    const transactions = await getRewardTransactions(userId) || []; //as the balance must be recalculated after user has done any transaction
    ...
    return Math.max(balance, 0); //never negative
}
...
export async function getAvailableRewards(userId: number){
  ...
  try {
    ...
    console.log('Fetching available rewards for the user:', userId);
